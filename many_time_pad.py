c = ["68 C0 21 F4 F1 0A 75 6C 1F E3 87 FE 44 F8 D6 EA \
...
for i in range(len(c)):
	c[i] = [chr(int(k, 16)) for k in c[i].split()]
...
for i in range(min(len(c[0]), len(c[1]))):
	c0_xor_c1.append( chr( ord(c[0][i]) ^ ord(c[1][i]) ) )

for i in range(min(len(c[2]), len(c[1]))):
	c1_xor_c2.append( chr( ord(c[1][i]) ^ ord(c[2][i]) ) )

for i in range(min(len(c[0]), len(c[2]))):
	c0_xor_c2.append( chr( ord(c[0][i]) ^ ord(c[2][i]) ) )
	
for i in range(min(len(c[0]), len(c[3]))):
	c0_xor_c3.append( chr( ord(c[0][i]) ^ ord(c[3][i]) ) )
	
for i in range(min(len(c[0]), len(c[4]))):
	c0_xor_c4.append( chr( ord(c[0][i]) ^ ord(c[4][i]) ) )

for i in range(min(len(c[6]), len(c[7]))):
	c6_xor_c7.append( chr( ord(c[6][i]) ^ ord(c[7][i]) ) )

for i in range(min(len(c[6]), len(c[0]))):
	c0_xor_c6.append( chr( ord(c[6][i]) ^ ord(c[0][i]) ) )

for i in range(min(len(c[0]), len(c[7]))):
	c0_xor_c7.append( chr( ord(c[0][i]) ^ ord(c[7][i]) ) )
	
for i in range(min(len(c[0]), len(c[10]))):
	c0_xor_c10.append( chr( ord(c[0][i]) ^ ord(c[10][i]) ) )


att = list(" the ") #try different strings

# by this we know that (m[0] ^ m[2])[5:10] is " the " ^ "as si". May try to decipher other messages
for i in range(len(c0_xor_c2) - len(att)):
	dec = ""
	for j in range(len(att)):
		dec += chr(ord(c0_xor_c2[i + j]) ^ ord(att[j]))
	print(dec, i)
